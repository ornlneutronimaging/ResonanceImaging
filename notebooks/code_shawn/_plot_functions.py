...
def get_sigma_iso_terms_from_database(isotopes, file_names, energy_min, energy_max, iso_ratio_list, sub_x):
    # Transmission calculation of summed and separated contributions by each isotopes
    """
    Function used to get sigma terms (sigma_b * isotopic ratios) with normalized x-axis
    :param isotopes: isotope list for single element.
                    ['58-Co', '59-Co'] for 'Co', ['107-Ag', '109-Ag'] for 'Ag'.
    :param file_names: file names of sigma data files in directory: data_web/ENDF_VIII/
                    ['58-Co.csv', '59-Co.csv']
    :param energy_min: minimum energy used to reduce the size extracted from sigma files
                    int or float
    :param energy_max: maximum energy used to reduce the size extracted from sigma files
                    int or float
    :param iso_ratio_list: isotopic ratios in list
                    [0.1, 0.2, 0.3]
                    float
    :param sub_x: steps used to interpolate energy axis for each isotope
                    int
    :return:
    x_energy: interpolated new x-axis
    sigma_iso_dict: dictionary of (sigma * isotopic ratio) for each isotopes
                    dictionary, float
    sigma_iso_sum: the summation of (sigma * isotopic ratio), thus for the specific element
                    list, float

    df_raw: the pd.DataFrame of the all raw sigma files for each isotope
    """
    df = pd.DataFrame()
    df_raw = pd.DataFrame()
    sigma_iso_dict = {}
    ...
    for i, iso in enumerate(isotopes):
        ...
        sigma_iso_dict[iso] = sigma_b * iso_at_ratio[i] #* ele_at_ratio
        # sigma_iso_ele_l_isodict[iso] = sigma_iso_ele_isodict[iso] * thick_cm
        sigma_iso_sum = sigma_iso_sum + sigma_b * iso_at_ratio[i] #* ele_at_ratio
    ...
    return x_energy, sigma_iso_dict, sigma_iso_sum, df_raw


def plot_resonance(_energy_x_axis, _trans_y_axis, _plot_mixed, _plot_each_ele_contribution, _plot_each_iso_contribution,
                   elements, isotope_dict, x_energy, y_sum, y_ele_dict, y_all_dict, _input_formula):
    """
    Plot the neutron resonance
    :param _energy_x_axis:
    use energy ('Y') or lambda ('N') as x-axis
    boolean

    :param _trans_y_axis:
    use transmission ('Y') or attenuation ('N') as y-axis
    boolean

    :param _plot_mixed:
    plot ('Y') or NOT plot ('N') the total resonance
    boolean

    :param _plot_each_ele_contribution:
    plot ('Y') or NOT plot ('N') the resonance of each element
    boolean

    :param _plot_each_iso_contribution:
    plot ('Y') or NOT plot ('N') the resonance of each isotope
    boolean

    :param elements:
    the list of element in string
    list
    ...
    :param x_energy:
    the interpolated x-axis from function: 'get_sigma_iso_terms_from_database'
    ...
    if _plot_each_iso_contribution == 'Y':
        # Plot resonance by isotopes
        for _ele in elements:
            for _iso in isotope_dict[_ele]:
                _y_each_axis = y_all_dict[_ele][_iso]
...
#
#
# def l_x_n_compound(elements, thick_cm, compound_density, molar_mass_dict, ele_at_ratio_dict):
#     molar_mass_sum = 0.
#     for ele in elements:
#         molar_mass_sum = molar_mass_sum + molar_mass_dict[ele] * ele_at_ratio_dict[ele]
#     l_x_n = thick_cm * compound_density / molar_mass_sum
